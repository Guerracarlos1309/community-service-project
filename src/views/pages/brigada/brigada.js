...
const BrigadeManagement = () => {
  ...
  const downloadPdf = async (endpoint, filename) => {
    try {
      ...
      console.log(`📄 Descargando PDF: ${endpoint}`)
      ...
      addToast(`PDF "${filename}" descargado exitosamente`, 'success')
      console.log(`✅ PDF descargado: ${filename}`)
    } catch (error) {
      console.error('❌ Error descargando PDF:', error)
      addToast(`Error al descargar PDF: ${error.message}`, 'danger')
    } finally {
      setDownloadingPdf(false)
    }
  }

  // Función para descargar PDF de brigada específica
  const downloadBrigadePdf = async (brigadeId, brigadeName) => {
    try {
      setDownloadingBrigadePdf((prev) => ({ ...prev, [brigadeId]: true }))
      setError(null)

      console.log(`📄 Descargando PDF de brigada: ${brigadeId}`)

      const token = localStorage.getItem('accessToken')
      const response = await fetch(`http://localhost:3001/api/pdf/brigades/${brigadeId}/details`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
      ...
      a.style.display = 'none'
      a.href = url
      a.download = `detalle_brigada_${brigadeName.replace(/\s+/g, '_')}.pdf`
      document.body.appendChild(a)
      a.click()
      window.URL.revokeObjectURL(url)
      document.body.removeChild(a)

      addToast(`PDF de brigada "${brigadeName}" descargado exitosamente`, 'success')
      console.log(`✅ PDF de brigada descargado: ${brigadeName}`)
    } catch (error) {
      console.error('❌ Error descargando PDF de brigada:', error)
      addToast(`Error al descargar PDF: ${error.message}`, 'danger')
    } finally {
      setDownloadingBrigadePdf((prev) => ({ ...prev, [brigadeId]: false }))
    }
  }
  ...
  const handleRemoveTeacher = async (brigade) => {
    try {
      if (
        !window.confirm(
          `¿Está seguro de que desea remover el docente de la brigada "${brigade.name}"?`,
        )
      ) {
        return
      }
  ...
  const totalStudents = brigades.reduce((sum, brigade) => sum + (brigade.studentCount || 0), 0)
  ...
  return (
    <>
        ...
        <CCardHeader className="bg-light d-flex justify-content-between align-items-center">
          <div>
            ...
            <small className="text-muted">
              Administra las brigadas estudiantiles y sus docentes encargados
            </small>
          </div>
          <div className="d-flex gap-2">
            {/* Dropdown para reportes PDF */}
            <CDropdown>
              <CDropdownToggle color="info" disabled={downloadingPdf} className="text-white">
                {downloadingPdf ? (
                  <>
              ...
              <CDropdownMenu>
                <CDropdownItem
                  onClick={() =>
                    downloadPdf('/api/pdf/brigades/list', 'listado_brigadas_docentes.pdf')
                  }
                  disabled={downloadingPdf}
                >
                  <CIcon icon={cilPrint} className="me-2" />
                  Listado de Brigadas y Docentes
                </CDropdownItem>
              </CDropdownMenu>
            </CDropdown>
                    ...
                    <CCardBody className="d-flex flex-column p-4">
                      <div className="flex-grow-1">
                        ...
                        <div className="d-flex justify-content-between align-items-start mb-3">
                          ...
                          <CBadge
                            color={brigade.encargado_name ? 'success' : 'warning'}
                            shape="rounded-pill"
                          >
                        ...
                        <div className="mb-3">
                          ...
                          {brigade.encargado_ci && (
                            <small className="text-muted">CI: {brigade.encargado_ci}</small>
                          )}
                        </div>
                      ...
                      <div className="d-grid gap-2">
                        ...
                        {/* Botón para descargar PDF de la brigada */}
                        <CButton
                          color="secondary"
                          size="sm"
                          onClick={() => downloadBrigadePdf(brigade.id, brigade.name)}
                          disabled={downloadingBrigadePdf[brigade.id]}
                          className="text-white"
                        >
                          {downloadingBrigadePdf[brigade.id] ? (
                            <>
                              <CSpinner size="sm" className="me-1" />
                              Descargando...
                            </>
                ...
                <div className="text-center py-5">
                  ...
                  <p className="text-muted mb-4">
                    {searchTerm
                      ...
                      : 'Comienza creando tu primera brigada estudiantil'}
                  </p>
                  {!searchTerm && (
          ...
          <div className="text-center">
            ...
            <div className="alert alert-danger">
              ...
              <ul className="mt-2 mb-0 text-start">
                ...
                <li>Las asignaciones de docentes</li>
      ...
      <CModal visible={showDetailsModal} onClose={() => setShowDetailsModal(false)} size="xl">
                ...
                <CCardBody>
                  {brigadeStudents.length > 0 ? (
                    ...
                    <div className="text-center py-4">
                      ...
                      <p className="text-muted">Esta brigada aún no tiene estudiantes asignados.</p>
                      <CButton
                        color="primary"
                        onClick={() => {
        ...
        <CModalFooter className="bg-light">
          ...
          {selectedBrigade && (
            <CButton
              color="info"
              onClick={() => downloadBrigadePdf(selectedBrigade.id, selectedBrigade.name)}
              disabled={downloadingBrigadePdf[selectedBrigade.id]}
              className="text-white"
            >
        ...
        <CModalBody className="p-4">
          <CForm>
            <div className="mb-4">
              ...
              <small className="text-muted">
                Solo se muestran docentes disponibles en el sistema
              </small>
            </div>
            <div className="mb-4">
              ...
              />
              <small className="text-muted">
                Fecha en que el docente comenzará a encargarse de la brigada
              </small>
            </div>
          </CForm>
        </CModalBody>
      ...
      <CModal
        visible={showEnrollStudentsModal}
            ...
            <div className="d-flex justify-content-between align-items-center mb-3">
              ...
              <div className="text-end">
                <div className="fw-semibold text-primary">
                  {studentForm.studentIds.length} seleccionados
                </div>
            ...
            <CProgress
              value={
                availableStudents.length > 0
                  ? (studentForm.studentIds.length / availableStudents.length) * 100
                  : 0
              }
              color="primary"
              className="mb-3"
          ...
          {availableStudents.length > 0 ? (
            ...
            <div className="text-center py-5">
              ...
              <p className="text-muted">
                Todos los estudiantes activos ya están asignados a brigadas o no hay estudiantes
                registrados.
              </p>
            </div>
          )}
